...
const { Status, TemperatureUnit } = require('./Enums');
...
class Data {
  ...
  fromApiData(raw) {
    ...
    // Needed for temperature conversion
    if ('temperature_unit' in raw) this.temperature_unit = TemperatureUnit[raw.temperature_unit];
    ...
    if ('sensor' in raw) {
      if ('id' in raw.sensor) data.sensor_id = raw.sensor.id;
  ...
  measureValue(property, measurements) {
    ...
    let value = measurements[property].values[field] || null;

    if (property === 'temperature' && value !== null && this.temperature_unit === 'F') {
